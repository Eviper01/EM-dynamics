...
class Particle():
    ...
    def accelerate(self, Ex, Ey, Ez):
        qEy, qEx, qEz = np.gradient(self.columb_potential())
        nEx = Ex + qEx
        nEy = Ey + qEy
        nEz = Ez + qEz
        try:
            ...
            self.velocity[2] += nEz[self.pindex()[1], self.pindex()[0]]*self.charge/self.mass
        except IndexError:
            pass
    ...
    def pindex(self):
        ...
        idz = (np.abs(z - self.position[2])).argmin()
        return idx, idy, idz
